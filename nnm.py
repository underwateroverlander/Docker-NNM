...
activation_code = input('Please press enter and input your activation code. If connecting to Tenable.sc, enter SecurityCenter.')
...
build_docker = subprocess.run([
    "docker", "exec", "nnm-local", "bash", "-c", "./usr/bin/activationcode.txt"])
#Set Monitored Range
nnm_range = subprocess.run([
    "docker", "exec", "nnm-local", "/opt/nnm/bin/nnm", "--config", "Monitored Network IP Addresses and Ranges", "0.0.0.0/8"])
#Remove Complex Passwords
nnm_pw = subprocess.run([
    "docker", "exec", "nnm-local", "/opt/nnm/bin/nnm", "--config", "Enforce Complex Passwords", "0"])
#Monitor Mode all the way
nnm_monitor = subprocess.run([
    "docker", "exec", "nnm-local", "/opt/nnm/bin/nnm", "--config", "Run in Discovery Mode", "0"])
#Change password
change_pw = subprocess.run([
    "docker", "exec", "nnm-local", "/opt/nnm/bin/nnm", "--users", "--chpasswd", "admin", "password"])
#Force NNM to start
start_nnm = subprocess.run([
    "docker", "exec", "-d", "nnm-local", "/opt/nnm/bin/nnm"])
